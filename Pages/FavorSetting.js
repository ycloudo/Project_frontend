...
const FavorSetting = ({ navigation }) => {
  ...
  const click = () => {
    counter = 0;
    if (FSstate.food === true) {
      counter += 1;
    }
    if (FSstate.price === true) {
      counter += 1;
    }
    if (FSstate.env === true) {
      counter += 1;
    }
    if (FSstate.service === true) {
      counter += 1;
    }
    if (FSstate.traffic === true) {
      counter += 1;
    }
    if (counter == 2) {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.option_row}>
        <View style={styles.option}>
          <Image source={require('../assets/food.png')} style={styles.photo} />
          <CheckBox
            title="食物"
            checked={FSstate.food}
            onPress={() => checkBoxHandler({ food: !FSstate.food })}
        ...
        <View style={styles.option}>
          <Image source={require('../assets/money.png')} style={styles.photo} />
          <CheckBox
            title="價錢"
            checked={FSstate.price}
            onPress={() => checkBoxHandler({ price: !FSstate.price })}
            checkedIcon="dot-circle-o"
            uncheckedIcon="circle-o"
            containerStyle={{
              backgroundColor: '#FDF6DD',
        ...
        <View style={styles.option}>
          ...
          <CheckBox
            title="環境"
            checked={FSstate.env}
            onPress={() => checkBoxHandler({ env: !FSstate.env })}
            checkedIcon="dot-circle-o"
            uncheckedIcon="circle-o"
            containerStyle={{
              backgroundColor: '#FDF6DD',
      ...
      <View style={styles.option_row}>
        <View style={styles.option}>
          ...
          <CheckBox
            title="服務"
            checked={FSstate.service}
            onPress={() => checkBoxHandler({ service: !FSstate.service })}
            checkedIcon="dot-circle-o"
            uncheckedIcon="circle-o"
            containerStyle={{
              backgroundColor: '#FDF6DD',
        ...
        <View style={styles.option}>
          ...
          <CheckBox
            title="交通"
            checked={FSstate.traffic}
            onPress={() => checkBoxHandler({ traffic: !FSstate.traffic })}
            checkedIcon="dot-circle-o"
            uncheckedIcon="circle-o"
            containerStyle={{
              backgroundColor: '#FDF6DD',
      ...
      <View style={styles.next}>
        <TouchableOpacity onPress={click}>
          <Icon_next name="arrowright" size={50} color="#7B7B7B" />
        </TouchableOpacity>
      </View>
    </View>
  );
